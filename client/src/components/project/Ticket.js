...
const Ticket = ({auth,auth:{user},getProject,removeTicket,editTicket,getMembers,getTicket, getTickets,removeMember,loading,profile: {profiles},getProfiles,project:{project,ticket, tickets, members}, match}) => {
  useEffect(() => {
...
const updateIndex = project.tickets.map(ticket => ticket._id.toString()).indexOf(match.params.id);


    return  members === null || user === null ||  project.projectName === null || ticket == null || ticket.name == null? <Spinner/> : <Fragment>
       {/* <p>{project.name}</p> */}
...
{/* NOTE SHOW MEMBERS STARTS HERE */}
  ...
  <div style={{overflowY : 'auto',height: '41rem'}}>
            ...
            {ticket.comments.map(comment => (
                <TicketCommentItem key={comment._id} comment={comment} ticketId={ticket._id}/>
            ))}
        </div>
        </div>
...
export default connect(mapStateToProps, {getCurrentProfile,editTicket,getTicket,getMembers,getTickets,removeTicket,removeMember,getProfiles,getProject}) (Ticket)
